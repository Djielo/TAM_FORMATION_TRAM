# Évolutions prévues — Révision RCT

Document de référence pour la suite du développement.

**État actuel (données)** : chapitre **Acronymes** (`data-rct-acronymes.js`) + 4 chapitres RCT dans `data-rct-ch1.js` … `ch4.js` — **91 modules**, **448 questions** QCM (choix multiples + correction).

**État actuel (interface)** : application statique dans `docs/` — onglets **Révision** (QCM par module) / **Pré-examen** (cartes par chapitre + SRS) / **Examen final** (cartes sur tout le RCT + note), déverrouillage progressif, persistance `localStorage`, modales d'aide par mode. Fichiers principaux : `app.js`, `progress.js`, `pool.js`, `pretest-session.js`, `dialog.js`, `app.css`.
...
│   ├── A.1 — Liste fournie par le RCT (sigles liste document RCT)
│   ├── A.2 — Matériel, circulation et urgence
│   └── A.3 — Signalisation et zones spécifiques
├── Sac 1 — Utilisation du matériel roulant (RCT p. 3–19)
...
Contenu RCT ch. 1–4 : **fait** (voir `data.js` + fichiers par chapitre).  
Chapitre **Acronymes** : **fait** — conseil d’accueil et pop-up Révision pour commencer par ce chapitre ; module **A.1** intitulé **« Liste fournie par le RCT »** (pas « liste officielle »). Voir `docs/REDIGER-QUESTIONS-RCT.md`.  
Évolutions ponctuelles possibles si retour terrain (formateurs / conducteurs).

### Qualité des énoncés

Les `prompt` initiaux étaient souvent au format **QCM télégraphique** (titre : complément …). **Chapitres 1 à 4** : reformulation en phrases complètes dans `data-rct-ch1.js` … `ch4.js`, à partir du contenu déjà présent dans chaque question (`prompt` / `choices` / `explanation`) — **sans extrapolation** de faits non écrits dans le jeu de données.
...
Guide local (non versionné) : **`docs/REDIGER-QUESTIONS-RCT.md`**.
...
| -------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| **Validées : X/Y** ou **🥳 X/Y** | `tam-rct-question-mastery-v1` (`everCorrect`, Y = taille actuelle du module) | Déverrouillage **404/404** et module « parfait ».                         |
| **Meilleur : X/Y**               | `tam-rct-revision-v1` (meilleur score QCM de session)                        | Mémoire du meilleur passage ; **conservé** si le RCT gagne des questions. |
| **QCM en cours (q. N)**          | `tam-rct-quiz-active-v1`                                                     | Session QCM interrompue.                                                  |
...
- Bandeau vert : **RCT**, **Consignes d'exploitation TaM**, indicateur de progression (voir ci-dessous).
...
**Périmètre** : Révision = **module** · Pré-examen = **chapitre** (un seul) · Examen final = **l'ensemble du RCT**.
...
| **Premier clic** (ou démarrage) **Pré-examen**            | Chapitre ; libellé quota : _« Choisissez votre objectif potentiel de cartes à maîtriser pour cette session. »_ ; session en cours vs nouvelle, SRS, **conseil de pause ≥ 5 min** ; **examen final** : _« Pour atteindre l'examen final. Vous devez maîtriser 80 % des réponses de chaque chapitre. »_ |
...
**Pied de page accueil Révision** : progression globale validées, compteur « une question comptabilisée dès qu’elle a été bien répondue au moins une fois », rappel persistance : _« Votre progression est enregistrée dans ce navigateur, sur cet appareil. Pour la retrouver, ouvrez toujours l’application avec le même lien (favori ou raccourci). »_ (origine navigateur = protocole + hôte + port ; pas synchronisé entre Chrome et Firefox).
...
**Formulation affichée** : _« Pour atteindre l'examen final. Vous devez maîtriser 80 % des réponses de chaque chapitre. »_
...
**Implémentation** : `getPretestChapterMastery(axisId)` dans `progress.js` (compte SRS par chapitre). `tam-rct-pretest-stats-v1` conserve encore les stats de fin de session (historique) mais **n’alimente plus** le déverrouillage de l’examen final.
...
- Ou : `localStorage.setItem('tam-rct-dev-unlock-v1', '1')` dans la console du navigateur.
...
- **Reset global** (implémenté) : **5 appuis rapides** sur le titre **RCT** dans l’en-tête (≤ 2,5 s) → modale de confirmation (`dialog.js`, bouton danger « Tout effacer »). Pas de bouton visible en bas de l’accueil (éviter les effacements accidentels).
...
Schéma actuel : **v3** (`tam-rct-storage-schema-v2`).

Au premier chargement après mise à jour du RCT dans le dépôt :
...
| Clé                             | Contenu                                                                                                                       |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `tam-rct-storage-schema-v2`     | Version de schéma (valeur **3**).                                                                                             |
| `tam-rct-revision-v1`           | Meilleur score QCM par module (`axisId/moduleId` → `{ score, total, at }`).                                                   |
| `tam-rct-question-mastery-v1`   | Par `questionId` : `everCorrect`, etc. — source **404/404** et 🥳 module.                                                     |
| `tam-rct-srs-v1`                | Par `questionId` : SRS pré-examen ; **`intervalIndex ≥ 1`** = « Je maîtrise » compté pour déverrouillage examen final.        |
| `tam-rct-quiz-active-v1`        | QCM Révision en cours (reprise auto au chargement).                                                                           |
| `tam-rct-pretest-prefs-v1`      | Dernier quota N choisi **par chapitre** (25…150).                                                                             |
| `tam-rct-pretest-active-v1`     | Session **en cours** par chapitre : file, index, `masterCount`, etc.                                                          |
| `tam-rct-pretest-last-end-v1`   | Fin de dernière session pré-examen **terminée** (avertissement 5 min **global**).                                             |
| `tam-rct-pretest-stats-v1`      | Historique fin de session (meilleur taux session / chapitre) — **informatif**, pas utilisé pour déverrouiller l’examen final. |
| `tam-rct-dev-unlock-v1`         | Bypass test formateur (`1`).                                                                                                  |
| `tam-rct-help-dismissed-v1`     | Modales d'aide déjà vues (par mode : `revision`, `pretest`, `final`).                                                         |
| `tam-rct-final-exam-prefs-v1`   | Dernier nombre de questions (Examen final).                                                                                   |
| `tam-rct-final-exam-active-v1`  | Session examen final en cours.                                                                                                |
| `tam-rct-final-exam-history-v1` | Historique des sessions examen final.                                                                                         |
...
- **Déverrouillage examen final** : formulation unique _« Pour atteindre l'examen final. Vous devez maîtriser 80 % des réponses de chaque chapitre. »_ ; quota session = _« Choisissez le nombre de cartes à maîtriser pour cette session. »_
- **Affichage modules Révision** : Validées / 🥳 / Meilleur / QCM en cours ; pas de « Validées : 0/Y ».
- **Bandeau RCT** : compteurs factuels uniquement (sans répéter les modales d’aide).
- **En-tête sticky** : bloc complet RCT + onglets (`.app-top-bar`).
- **Dialogues** : `dialog.js` (confirm / alert intégrés) ; réinitialisation cachée (5× RCT).
- **Migration v3** : prune des questions retirées du RCT ; conservation des scores modules QCM.
...
- Pas d’images en v1 : questions texte uniquement (photos du RCT peu exploitables en auto).
- Contenu = **RCT TaM EXP-CSG-01-17** ; outil d’entraînement personnel, pas substitut à la formation officielle.
